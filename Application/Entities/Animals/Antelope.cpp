...
void Antelope::Collision(Organism *defender, Organism *assulter) {
  ...
  if (chance <= 50) {
    ...
    if ((freeMove = GetFreePosition()) == STAY) {
      ...
      if (this == assulter) {
        std::cout << assulter->GetTypeToString() << " has fled from fight with " << defender->GetTypeToString()
                  << NEWLINE_CONSOLE;
        ...
        std::cout << defender->GetTypeToString() << " has fled from fight with " << assulter->GetTypeToString()
                  << NEWLINE_CONSOLE;
